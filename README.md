...
В данном проекте я разрабатываю свою библиотеку для обработки числовых матриц на языке программирования Си. Матрицы являются одной из базовых структур данных в программировании. Матрица - это набор чисел, расположенных в фиксированном количестве строк и столбцов.

## Реализация функции библиотеки my_c_matrix.h 

Реализованы описанные [ниже](#операции-над-матрицами) функции библиотеки my_c_math.h в соответствии со следующими требованиями:
 
- Библиотека разработана на языке Си с использованием компилятора gcc 
- Код библиотеки находиться в папке src
- Проект оформлен как статическая библиотека (с заголовочным файлом my_c_matrix.h)
...
- Разработан Makefile для сборки библиотеки и тестов с целями all, clean, test, my_c_matrix.a, gcov_report
- В цели gcov_report формируется отчёт gcov в виде html страницы. Для этого unit-тесты запускаться с флагами gcov 
- Матрица должна быть реализована в виде структуры описанной [ниже](#структура-матрицы-на-языке-c-для-данного-проекта) 
...
- Для создания библиотеки наберите в терминале следующую команду: **make my_c_matrix.a**
...
## Структура матрицы на языке C для данного проекта:
